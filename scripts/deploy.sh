...
if [ ! -z "$CIRCLE_BRANCH" ]; then
  ...
  GRAPHCOOL_PROJECT_ID_FROM_BRANCH="GRAPHCOOL_PROJECT_ID_${UPPER_BRANCH}"
  GRAPHCOOL_PAT=${!GRAPHCOOL_PAT_FROM_BRANCH:?$GRAPHCOOL_PAT_FROM_BRANCH env var not set}
  GRAPHCOOL_PROJECT_ID=${!GRAPHCOOL_PROJECT_ID_FROM_BRANCH:?$GRAPHCOOL_PROJECT_ID_FROM_BRANCH env var not set}
fi
