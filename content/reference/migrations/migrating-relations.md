...
You can add new relations to your GraphQL schema using the `@relation(name: String!)` tag. This will add new [mutations]() and modify existing [queries](), [mutations]() and [subscriptions]() in your GraphQL API.
...
* `user: User! @relation(name: "UserStories")` on the `Story` type signifies a [required relation field](). Singular relation fields can be optional or required.

By changing the multiplicities of the separate fields, we can create **one-to-one, one-to-many and many-to-many** relations.

Have a look at the [naming conventions for relations]() to see what names are allowed and recommended.
...
> Removing a relation potentially breaks existing queries, mutations and subscriptions. Make sure to not rely on this relation in your apps before deleting it.
...
> Renaming a relation potentially breaks existing queries, mutations and subscriptions. Make sure to adjust your app accordingly to the new name.
