...
The easiest way to get started with GraphQL React apps is using [`create-react-app`](https://github.com/facebookincubator/create-react-app) and [Apollo Client](http://dev.apollodata.com/).
...
Note that `ApolloProvider` is wrapping `Router`, which enables all child components to use access the functionality from Apoll Client for queries and mutations.
...
Taking all of that together, this is the `render` method that we end up with:
...
Similar to the `FeedQuery` before, we now have to replace the `export` statement with the following code snipped:
