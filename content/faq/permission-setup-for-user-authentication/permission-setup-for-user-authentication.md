...
Let's introduce an enum field `role` with the possible values `USER` and `MANAGER` to the `User` model for demonstration purposes. Then we can use the following authenticated [permission query](!alias-#query-permissions) with the `Create` operation on the `User` model.
